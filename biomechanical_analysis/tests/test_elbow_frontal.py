"""
Script para análisis de FLEXIÓN bilateral de codos (VISTA FRONTAL)
...
class ElbowFrontalAnalyzer:
    def __init__(self, processing_width=640, processing_height=480):
        """
        Inicializa el analizador para vista FRONTAL (Flexión bilateral de codos)
        ...
        self.max_left_elbow = 0
        self.max_right_elbow = 0
        self.frame_count = 0
    ...
    def calculate_elbow_angle(self, shoulder, elbow, wrist):
        ...
        - 180deg = Codo completamente flexionado
        ...
        """
        # Vector vertical fijo (referencia 0deg)
        vertical_down_vector = np.array([0, 1])
        
        # Vector del antebrazo (CODO → MUÑECA)
    ...
    def calculate_symmetry_difference(self):
        """Calcula la diferencia de simetría entre codos izquierdo y derecho"""
        return abs(self.left_elbow_angle - self.right_elbow_angle)
    ...
    def process_frontal_view(self, image, landmarks, w, h):
        """Procesa vista frontal - Análisis de flexión bilateral de codos"""
        ...
        # Calcular ángulos de flexión de ambos codos
        left_angle = self.calculate_elbow_angle(left_shoulder_2d, left_elbow_2d, left_wrist_2d)
        right_angle = self.calculate_elbow_angle(right_shoulder_2d, right_elbow_2d, right_wrist_2d)
        
        # Actualizar estadísticas
        self.left_elbow_angle = left_angle
        self.right_elbow_angle = right_angle
        
        if left_angle > self.max_left_elbow:
            self.max_left_elbow = left_angle
        if right_angle > self.max_right_elbow:
            self.max_right_elbow = right_angle
        
        # Dibujar puntos clave
        # CODO IZQUIERDO
        cv2.circle(image, left_elbow_2d, 10, self.color_cache['yellow'], -1, cv2.LINE_4)  # CODO
        cv2.circle(image, left_shoulder_2d, 8, self.color_cache['magenta'], -1, cv2.LINE_4)  # HOMBRO
        cv2.circle(image, left_wrist_2d, 8, self.color_cache['cyan'], -1, cv2.LINE_4)  # MUÑECA
        
        # CODO DERECHO
        cv2.circle(image, right_elbow_2d, 10, self.color_cache['yellow'], -1, cv2.LINE_4)
        cv2.circle(image, right_shoulder_2d, 8, self.color_cache['magenta'], -1, cv2.LINE_4)
        cv2.circle(image, right_wrist_2d, 8, self.color_cache['cyan'], -1, cv2.LINE_4)
        
        # Líneas de referencia verticales fijas (pasan por cada codo)
        vertical_length = 180
        
        # Eje vertical izquierdo
        left_vertical_start = (left_elbow_2d[0], left_elbow_2d[1] - vertical_length)
        left_vertical_end = (left_elbow_2d[0], left_elbow_2d[1] + vertical_length)
        cv2.line(image, left_vertical_start, left_vertical_end, self.color_cache['green'], 3, cv2.LINE_4)
        
        # Eje vertical derecho
        right_vertical_start = (right_elbow_2d[0], right_elbow_2d[1] - vertical_length)
        right_vertical_end = (right_elbow_2d[0], right_elbow_2d[1] + vertical_length)
        cv2.line(image, right_vertical_start, right_vertical_end, self.color_cache['green'], 3, cv2.LINE_4)
        
        # Líneas de los antebrazos (CODO → MUÑECA)
        cv2.line(image, left_elbow_2d, left_wrist_2d, self.color_cache['blue'], 4, cv2.LINE_4)
        cv2.line(image, right_elbow_2d, right_wrist_2d, self.color_cache['blue'], 4, cv2.LINE_4)
        
        # Brazos superiores (solo en CLEAN y FULL)
        if self.display_mode != "MINIMAL":
            cv2.line(image, left_shoulder_2d, left_elbow_2d, self.color_cache['blue'], 2, cv2.LINE_4)
            cv2.line(image, right_shoulder_2d, right_elbow_2d, self.color_cache['blue'], 2, cv2.LINE_4)
        
        # Mostrar ángulos
        left_angle_text = f"{left_angle:.1f}deg"
        cv2.putText(image, left_angle_text, 
                   (left_elbow_2d[0] - 70, left_elbow_2d[1] - 25),
        ...
        cv2.putText(image, right_angle_text, 
                   (right_elbow_2d[0] + 20, right_elbow_2d[1] - 25),
                   cv2.FONT_HERSHEY_SIMPLEX, 1.2, self.color_cache['yellow'], 3, cv2.LINE_4)
    ...
    def draw_info_panel(self, image, w, h):
        ...
        # Título
        cv2.putText(image, "ANALISIS DE FLEXION BILATERAL DE CODOS (FRONTAL)", (20, 40),
        ...
        # Ángulos actuales - CODO IZQUIERDO
        left_color = self.get_angle_color(self.left_elbow_angle)
        cv2.putText(image, f"Codo Izquierdo: {self.left_elbow_angle:.1f}deg", (20, 115),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, left_color, 2, cv2.LINE_4)
        cv2.putText(image, f"Max: {self.max_left_elbow:.1f}deg", (20, 145),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, self.color_cache['light_gray'], 2, cv2.LINE_4)
        
        # Ángulos actuales - CODO DERECHO
        right_color = self.get_angle_color(self.right_elbow_angle)
        cv2.putText(image, f"Codo Derecho: {self.right_elbow_angle:.1f}deg", (w//2 + 20, 115),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, right_color, 2, cv2.LINE_4)
        cv2.putText(image, f"Max: {self.max_right_elbow:.1f}deg", (w//2 + 20, 145),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, self.color_cache['light_gray'], 2, cv2.LINE_4)
        ...
        cv2.putText(image, f"Diferencia: {diff:.1f}deg [{diff_status}]", (20, 185),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, diff_color, 2, cv2.LINE_4)
        
        # Evaluación de simetría
        cv2.putText(image, "<10deg=Excelente | 10-20deg=Aceptable | >20deg=Revisar", (20, 215),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.color_cache['light_gray'], 1, cv2.LINE_4)
        
        # Instrucciones
        cv2.putText(image, "Flexiona ambos codos simultaneamente | 'R' reiniciar | 'M' cambiar modo | 'Q' salir", 
                   (20, h - 20),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, self.color_cache['white'], 2, cv2.LINE_4)
...
def main():
    """Función principal"""
    print("=" * 70)
    print("ANALISIS DE FLEXION BILATERAL DE CODOS - VISTA FRONTAL")
    print("=" * 70)
    ...
    print("   2. Flexiona AMBOS codos simultaneamente")
    print("   3. El sistema mide AMBOS codos y compara simetria")
    ...
    print("\n[*] SISTEMA GONIOMETRO:")
    print("   - 0deg = Brazos EXTENDIDOS (antebrazos hacia abajo)")
    print("   - 90deg = Antebrazos HORIZONTALES")
    print("   - 150deg = FLEXION MAXIMA (manos tocan hombros)")
    print("\n[*] ANALISIS DE SIMETRIA:")
    print("   - Verde: Diferencia < 10deg (EXCELENTE)")
    print("   - Naranja: Diferencia 10-20deg (ACEPTABLE)")
    print("   - Rojo: Diferencia > 20deg (REVISAR - posible asimetria)")
    print("=" * 70)
    
    # Inicializar analizador
    analyzer = ElbowFrontalAnalyzer(processing_width=640, processing_height=480)
    ...
    while cap.isOpened():
        ...
        # Mostrar frame
        cv2.imshow('Analisis de Codo - Vista Frontal', annotated_frame)
    ...
    print(f"   - Flexion maxima codo izquierdo: {analyzer.max_left_elbow:.1f}deg")
    print(f"   - Flexion maxima codo derecho: {analyzer.max_right_elbow:.1f}deg")
    
    # Análisis de simetría
    diff = abs(analyzer.max_left_elbow - analyzer.max_right_elbow)
    print(f"   - Diferencia entre codos: {diff:.1f}deg")
    ...
    if diff < 10:
        ...
        print(f"   - Recomendacion: Evaluar posible debilidad o lesion")
    
    # Evaluación del ROM promedio
    avg_rom = (analyzer.max_left_elbow + analyzer.max_right_elbow) / 2
    if avg_rom >= 140:
        print(f"   - ROM promedio: EXCELENTE ({avg_rom:.1f}deg)")
    elif avg_rom >= 120:
        print(f"   - ROM promedio: BUENO ({avg_rom:.1f}deg)")
    elif avg_rom >= 90:
        print(f"   - ROM promedio: LIMITADO ({avg_rom:.1f}deg)")
    else:
        print(f"   - ROM promedio: MUY LIMITADO ({avg_rom:.1f}deg) - Revisar")
